...
t.add_resource(ecs.Service(
    ...
    #     )
    # ),
    Cluster=ImportValue(Join("-",  [Select(0, Split("-Service-CF-Stack", Ref("AWS::StackName"))), "Cluster-id"])),
    DesiredCount=1,
    ...
    LoadBalancers=[ecs.LoadBalancer(
        ...
        #     Join(
        #         "-",
        #         [Select(0, Split("-Service-CF-Stack", Ref("AWS::StackName"))),
        #             "alb-helloworld-target-group"]
        #     ),
        # ),
        TargetGroupArn=ImportValue(Join("-", [Select(0, Split("-Service-CF-Stack", Ref("AWS::StackName"))), "ALB-target-group"])),
    )],
